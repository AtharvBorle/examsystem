...
const isImageStr = (str: string): boolean => {
  if (!str) return false
  const s = str.trim()
  return s.startsWith('data:image/') || /^https?:\/\//i.test(s)
}

const renderContent = (val: string, style?: React.CSSProperties) => {
  if (!val) return null
  if (isImageStr(val)) {
    return (
      <img 
...
function StandaloneSchoolDetailView({ schoolUdise, token, onBackToDashboard }: { schoolUdise: string; token: string | null; onBackToDashboard: () => void }) {
...
/* ==========================================
   VIEW: LOGIN
   ========================================== */
function LoginView({ onViewRegister, lang, onChangeLang }: { onViewRegister: () => void; lang: Language; onChangeLang: (lang: Language) => void }) {
  const { login } = useAuth()
...
/* ==========================================
   ...
   ========================================== */
function RegisterView({ onViewLogin, lang, onChangeLang }: { onViewLogin: () => void; lang: Language; onChangeLang: (lang: Language) => void }) {
  const { login } = useAuth()
...
/* ==========================================
   ...
   ========================================== */
function SuperAdminDashboard({ token }: { token: string | null }) {
...
function AdminDashboard({ token, lang }: { token: string | null; lang: Language }) {
...
/* ==========================================
   ...
   ========================================== */
function StudentDashboard({ token, user, lang }: { token: string | null; user: User; lang: Language }) {
  const [exams, setExams] = useState<any[]>([])
